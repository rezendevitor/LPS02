...
public class TMLeiturista extends AbstractTableModel{
    private ArrayList<Leiturista> lstLeituristas;
    ...
    public TMLeiturista(ArrayList<Leiturista> lst) {
        this.lstLeituristas = lst;
    }

    
    @Override
    public int getRowCount() {
        return lstLeituristas.size();
    }
    ...
    public Object getValueAt(int row, int col) {
        Leiturista usr = this.lstLeituristas.get(row);
        if(col == COL_NOME){
            return usr.getNome();
        }else if(col == COL_CPF){
            return usr.getCpf();
